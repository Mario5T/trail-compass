...
import { View, StyleSheet, Animated, Easing, Share } from "react-native";
import {
  Appbar,
  Card,
  Text,
...
export default function CompassScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const askForPermission = async () => {
      ...
      try {
        ...
        setSnack("Failed to get location");
      }
  ...
  const copyCoords = async () => {
    ...
    // TODO(3): Clipboard.setStringAsync("lat, lon") then snackbar
      const coordsString = `${fmt(coords.latitude)}, ${fmt(coords.longitude)}`;
  ...
  const shareCoords = async () => {
    ...
    // TODO(4): Share.share with message including coords + heading + cardinal
      ...
      const message = `I am here: ${fmt(coords.latitude)}, ${fmt(coords.longitude)} (${cardinal} ${Math.round(heading ?? 0)}°)`;
